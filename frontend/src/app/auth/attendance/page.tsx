...
//   firstName: string;
//   lastName: string;
//   role: number; // 2 for HR, 3 for employee
// };
...
//   const [inTime, setInTime] = useState<string>(''); // expects time in HH:MM format
//   const [outTime, setOutTime] = useState<string>(''); // expects time in HH:MM format
//   const [attendanceRecords, setAttendanceRecords] = useState<AttendanceRecord[]>([]);
//   const [users, setUsers] = useState<User[]>([]);
//   const router = useRouter();

//   // Helper: Combine selected time (HH:MM) with today's date
...
//   // Decode token and set logged-in user; for employees, set selectedUserId automatically.
//   useEffect(() => {
...
//   // For HR: fetch list of regular users (role === 3)
//   useEffect(() => {
...
//   // When selectedUserId changes, fetch that user's attendance records.
//   useEffect(() => {
//     if (selectedUserId) {
...
//   // For employees: check if today's attendance has already been marked.
...
//       // Refresh attendance records after adding a new record.
//       if (selectedUserId) {
//         axios.get<AttendanceRecord[]>(`http://localhost:5000/api/attendance/${selectedUserId}`)
//           .then(res => setAttendanceRecords(res.data))
//           .catch(err => console.error("Error fetching attendance records:", err));
...
//       {loggedInUser.role === 2 ? (
//         // HR view: dropdown for selecting an employee and viewing/adding attendance.
//         <div>
...
//         </div>
//       ) : (
//         // Employee view: show their attendance records and allow marking attendance (if not already marked today).
